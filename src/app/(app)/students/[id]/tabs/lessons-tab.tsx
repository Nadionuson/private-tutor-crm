...
function LessonRow({ lesson, student }: { lesson: LessonWithNotes; student: StudentWithStats }) {
  ...
  return (
    <div className={`grid grid-cols-[90px_1fr_auto_auto] items-start gap-3 px-5 py-3.5 border-b border-indigo-50 last:border-0 ${isUpcoming ? 'bg-indigo-50/30' : ''}`}>
      <div>
            ...
            <div className="flex gap-1.5 flex-wrap">
              ...
              {lesson.lesson_notes.outcome_notes && (
                <span className="text-[10px] bg-green-50 text-green-600 px-2 py-0.5 rounded">
                  Outcome: {lesson.lesson_notes.outcome_notes.slice(0, 40)}{lesson.lesson_notes.outcome_notes.length > 40 ? '…' : ''}
                </span>
              )}
            </div>
          </div>
